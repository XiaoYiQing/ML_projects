...
load_LPL_filestem = 'linPolyLin_data'
load_fileext = '.npz'
...
load_LPL_FFN = load_dir + '/' + load_LPL_filestem + load_fileext
...
y_LPL_cnt = len( y_LPL )        # Total LinPolyLin set data count.
...
# LPL data set partition indexing generation.
LPL_tr_len = math.floor( y_LPL_cnt*tr_ratio )
LPL_ts_len = y_LPL_cnt - LPL_tr_len
LPL_tr_idx, LPL_ts_idx = rand_samp( y_LPL_cnt, LPL_tr_len )
...
# Training and testing sets creation (logistic drop).
...
# Training and testing sets creation (linear-polynomial-linear drop).
y_LPL_tr = y_logis[ LPL_tr_idx ]
y_LPL_ts = y_logis[ LPL_ts_idx ]
labels_LPL_tr = labels_logis[ LPL_tr_idx ]
labels_LPL_ts = labels_logis[ LPL_ts_idx ]
...
# ======================================================================= >>>>>
#       Model Generation and Testing (Lin vs LPL)
# ======================================================================= >>>>>
...
model_fullfilename = save_dir + '/script_lin_vs_LPL_data_train.keras'

# Create a complete set of linear and logistic plot data for training.
X_tr = np.concatenate( [ y_lin_tr, y_LPL_tr ], axis=0 )
y_tr = np.concatenate( [ labels_lin_tr, labels_LPL_tr ], axis=0 )
# shuffle
...
X_ts = np.concatenate( [ y_lin_ts, y_LPL_ts ], axis=0 )
y_ts = np.concatenate( [ labels_lin_ts, labels_LPL_ts ], axis=0 )
...
# ======================================================================= >>>>>
#       Model Generation and Testing (Logis vs LPL)
# ======================================================================= >>>>>
...
load_from_save = False
save_dir = currentdir + '/ML_model_deposit'
model_fullfilename = save_dir + '/script_logis_vs_LPL_data_train.keras'

# Create a complete set of linear and logistic plot data for training.
X_tr = np.concatenate( [ y_logis_tr, y_LPL_tr ], axis=0 )
y_tr = np.concatenate( [ labels_logis_tr, labels_LPL_tr ], axis=0 )
# shuffle
idx = np.random.permutation( len( X_tr ) )
X_tr, y_tr = X_tr[idx], y_tr[idx]
# add channel dimension (Initalize at one for greyscale).
...
# Create a complete set of linear and logistic plot data for testing.
X_ts = np.concatenate( [ y_logis_ts, y_LPL_ts ], axis=0 )
y_ts = np.concatenate( [ labels_logis_ts, labels_LPL_ts ], axis=0 )
...
# ======================================================================= >>>>>
#       Model Generation and Testing (Lin vs LPL and Logis)
# ======================================================================= >>>>>
...
load_from_save = False
save_dir = currentdir + '/ML_model_deposit'
model_fullfilename = save_dir + '/script_lin_vs_other_data_train.keras'

# Labels manipulation.
labels_lin_tr = np.ones( len( labels_lin_tr ) )
labels_LPL_tr = np.zeros( len( labels_LPL_tr ) )
labels_logis_tr = np.zeros( len( labels_logis_tr ) )

# Create a complete set of linear and logistic plot data for training.
X_tr = np.concatenate( [ y_lin_tr, y_LPL_tr, y_logis_tr ], axis=0 )
y_tr = np.concatenate( [ labels_lin_tr, labels_LPL_tr, labels_logis_tr ], axis=0 )
# shuffle
idx = np.random.permutation( len( X_tr ) )
X_tr, y_tr = X_tr[idx], y_tr[idx]
# add channel dimension (Initalize at one for greyscale).
...
# Labels manipulation.
labels_lin_ts = np.ones( len( labels_lin_ts ) )
labels_LPL_ts = np.zeros( len( labels_LPL_ts ) )
labels_logis_ts = np.zeros( len( labels_logis_ts ) )

# Create a complete set of linear and logistic plot data for testing.
X_ts = np.concatenate( [ y_lin_ts, y_LPL_ts, y_logis_ts ], axis=0 )
y_ts = np.concatenate( [ labels_lin_ts, labels_LPL_ts, labels_logis_ts ], axis=0 )
